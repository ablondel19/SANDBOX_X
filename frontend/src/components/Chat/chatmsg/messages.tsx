  ...
  async function loadAvatar(login_rec) {
    try {
      const response = await axios.get(
        `http://localhost:3001/app/users/avatar/${login_rec}`,
      );
    ...
    <div>
      {chatMessages[0] ? (
        <ScrollArea style={{ height: 450 }} scrollbarSize={0}>
          {chatMessages.map(
            (
              elem: { message: string; userID: string; createdAt: Date },
              index,
            ) => (
              <div
                key={index}
                ...
                className="message"
                style={
                  data.getByLogin.blackList.includes(elem.userID)
                    ? { filter: 'blur(3px)' }
                    : { filter: 'blur(0px)' }
                }
              >
                {elem.userID === currentLogin ? (
                  ...
                  <>
                    ...
                    <Avatar
                      size={40}
                      color="blue"
                      src={`data:image/jpeg;base64,${getAvatar(elem.userID)}`}
                    ></Avatar>
                  </>
                )}
              </div>
      ...
      {lastMute && moment.default().diff(lastMute.mutedUntil, 'minutes') < 0 ? (
        ...
        >
          You are muted for{' '}
          {Math.abs(moment.default().diff(lastMute.mutedUntil, 'minutes'))}{' '}
          minutes{' '}
        </Text>
      ) : (
