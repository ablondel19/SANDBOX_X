...
const ChatBox = ({ uuid }: any) => {
  ...
  function sendMsg() {
    if (value.length > 0) {
      addMessage({
        variables: {
          newMessage: { chatUUID: uuid, userID: currentLogin, message: value },
        },
      }).catch((err) => {
  ...
  return (
    <Stack
      sx={{ height: '8vh', bottom: 0, position: 'absolute' }}
      justify="center"
      p={0}
    >
      <Group position="right">
        ...
        <ActionIcon
          ...
          size="lg"
          onClick={() => sendMsg()}
          disabled={!/\S/.test(value) ? true : value.length < 1 ? true : false}
        >
          <Send />
        </ActionIcon>
      </Group>
