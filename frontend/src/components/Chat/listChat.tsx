...
import { AiFillLock, AiOutlineDelete, AiOutlineSetting } from 'react-icons/ai';
                ...
                .map((elem) => (
                  <div key={elem.uuid} style={{ padding: '5px' }}>
                    <Group className="chat-group">
                      <UnstyledButton
                        onClick={() => {
                          const lastBan = elem.getBan.find(
                            (elem) => elem.login === currentLogin,
                          );
                          if (
                            lastBan &&
                            moment
                              .default()
                              .diff(lastBan.bannedUntil, 'minutes') < 0
                          ) {
                            toast.error(
                              'You are banned from this group for ' +
                                Math.abs(
                                  moment
                                    .default()
                                    .diff(lastBan.bannedUntil, 'minutes'),
                                ).toString() +
                                ' minutes',
                              {
                                position: 'top-center',
                ...
                .map((elem) => (
                  <div key={elem.uuid} style={{ padding: '5px' }}>
                    <Group className="chat-group">
                      <UnstyledButton
                        onClick={() => {
                          elem.name = elem.userID.filter(
                            (elem) => elem !== currentLogin,
                          );
                          currentChatVar(elem);
                        ...
                        <Group>
                          <Avatar size={40} color="blue">
                            {elem.password ? (
                              ...
                              elem.userID
                                .filter((elem) => elem !== currentLogin)[0]
                                .slice(0, 2)
                                .toUpperCase()
                            )}
                          </Avatar>
